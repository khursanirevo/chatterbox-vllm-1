#!/usr/bin/env python3
"""
Generate audio samples for FP32 mode (baseline) for quality comparison.

This script creates short, medium, and long audio samples using FP32 mode
so you can compare with the FP16 samples.
"""
...
async def generate_samples():
    """Generate audio samples with FP32 mode."""

    print("="*60)
    print("Generating FP32 Audio Samples (Baseline)")
    print("="*60)
    ...
    # Initialize model with FP32
    print("\nInitializing model with FP32 (baseline)...")
    model = await ChatterboxTTSAsync.from_pretrained(
        max_batch_size=16,
        max_model_len=1000,
        s3gen_use_fp16=False,  # FP32 baseline
    ...
    output_dir = Path("fp32_samples")
    output_dir.mkdir(exist_ok=True)

    # Test prompts (same as FP16 for fair comparison)
    prompts = {
    ...
    for category, text in prompts.items():
        ...
        try:
            ...
            if audio and len(audio) > 0:
                # Save audio
                output_path = output_dir / f"fp32_{category}.wav"
    ...
    print("\n" + "="*60)
    print("✅ All FP32 samples generated successfully!")
    print("="*60)
    ...
    print("\nComparison:")
    print("  FP16 samples: fp16_samples/")
    print("  FP32 samples: fp32_samples/")
    print("\nListen to compare quality:")
    print(f"  ffplay {output_dir}/*.wav")
